...
Direct fetching from NSE XBRL URLs is blocked or requires session cookies and browser headers. To reliably fetch data, we use a **local XML file** (downloaded manually).

Corporate filings from NSE: https://www.nseindia.com/companies-listing/corporate-filings-financial-results

Instructions to Run Locally:
1. Download the Infosys XBRL file from NSE and save it locally as `infosys.xml`.
2. Ensure Python is installed.
3. Install required packages:
   pip install streamlit pandas numpy xmltodict
4. Navigate to the directory containing this file.
5. Run the application:
   streamlit run <filename>.py
6. Open the browser at http://localhost:8501 to view the UI.
"""
...
# -----------------------------
# Fetch fundamentals from local NSE XBRL file
# -----------------------------

LOCAL_XBRL_FILE = "infosys.xml"

def fetch_fundamentals_from_xbrl(filepath: str) -> Dict[str, Any]:
    try:
        ...
        print(f"Error reading XBRL data from local file: {e}")
        return {
            'symbol': 'INFY',
            'longName': 'Infosys Limited',
            'marketCap': None,
...
def enrich_universe(df_symbols: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, r in df_symbols.iterrows():
        symbol = r["symbol"]
        fund = fetch_fundamentals_from_xbrl(LOCAL_XBRL_FILE)
        row = {**r.to_dict(), **fund}
...
def streamlit_app():
    ...
    st.title("Indian Stock Screener — NSE XBRL Data (Local File)")
